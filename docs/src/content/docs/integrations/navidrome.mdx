...
## Playlists held in Navidrome

With `AURRAL_NAVIDROME_USER_AUTH=reverse-proxy`, Library → Playlists shows the signed-in
user's Navidrome playlists and edits them in place: New playlist, Rename, Delete, Add to
playlist from any track, and remove from the open playlist. Every change is a Subsonic call
made as that user, so it is visible in Navidrome and in every Subsonic client at once, and a
playlist edited in a Navidrome client shows the change here on the next load. Aurral stores
nothing for these playlists.

Aurral matches tracks to Navidrome songs by file path. The first successful match teaches it
where the two mount the same library; after that lookups are exact. Tracks that Navidrome has
not indexed cannot be added, and playlist entries with no file on this server are listed but
cannot play.

Requirements:

- Navidrome must trust Aurral's address for external auth. Set `ND_EXTAUTH_TRUSTEDSOURCES`
  (older releases: `ND_REVERSEPROXYWHITELIST`) to include Aurral's container address or its
  Docker network in CIDR form, and keep `ND_EXTAUTH_USERHEADER` in step with
  `AURRAL_NAVIDROME_USER_HEADER` (both default to `Remote-User`).
- Aurral usernames must equal Navidrome usernames, which an SSO setup usually guarantees.
- The admin Navidrome connection under Settings → Playback stays in use for path lookups.

Pair it with `AURRAL_AUTOMATIC_PLAYLISTS_ENABLED=false` when only hand-made playlists are
wanted.

## Ratings and stars held in Navidrome

With `AURRAL_NAVIDROME_USER_AUTH` on, star ratings appear on library track rows, on the
open playlist, and in the player bar. A rating is written to Navidrome as the signed-in user
and read back the same way, so it is the rating every Navidrome client shows. Clicking the
current rating clears it. Hearting a track or album in Aurral also stars it in Navidrome, and
unhearting removes the star; that mirroring is best effort and never blocks the favourite.

Ratings need the same track-to-song link as playlists, so tracks Navidrome has not indexed
show no stars.

Stars work in both directions. A star added in the Navidrome web player or on a phone is
taken into Aurral's favourites the next time the favourites are read, at most once every few
minutes per person. Only additions cross: a favourite in Aurral with no star in Navidrome may
simply be a track Navidrome does not hold, and removing it would lose it for good. To pull
them in at once, `POST /api/navidrome-ratings/favorites/import`.

## Track identity

Aurral and Navidrome index the same files, so the file is the identity. Navidrome addresses
it by a song id, unique per library, and Aurral by an absolute path. Each link is worked out
once, from an exact path query, and then kept in the database, so later reads cost nothing and
a song id arriving from Navidrome, such as a star, becomes a file without asking Navidrome at
all. Links survive restarts. A file removed and re-added on the Navidrome side gets a new id,
and the stale one is resolved again on the next miss.

Set `AURRAL_NAVIDROME_MUSIC_ROOT` to state where Navidrome's main library sits in Aurral's
filesystem. Without it the mapping is learned from the first file matched on both sides, which
works but costs an extra search and relies on a guess.

Playlist reads prefer, in order: a link already stored, the path the player itself reported,
and the admin connection. Navidrome only sends real paths to players configured to receive
them, and a reported path is used only when it matches an indexed file whose title agrees.

## Smart playlists

A smart playlist is written as rules rather than as a list of songs, and Navidrome keeps it up
to date. Aurral offers a rule editor: match all or any of a set of rules, each one a field, a
comparison and a value, with an optional limit and a way of choosing which tracks fill it.

The fields on offer come from the server rather than from the editor, because Navidrome answers
a rule about a field it does not know with an empty playlist and no error, so a typo would look
like a rule that matches nothing. Rules written elsewhere that use something the editor cannot
show, a nested group for instance, are left alone: the playlist works, it simply cannot be
opened here.

Making one takes both connections. The playlist is created as the signed-in user, so it belongs
to them, and the rules are attached through the admin connection, which is the only one allowed
to write them. A smart playlist keeps no stored entries, so its tracks are looked up by id and
its rows cannot be reordered or removed.

### Bringing iTunes smart playlists across

`scripts/itunes-migrate/smart_playlists.py` reads the rules out of an iTunes library export and
writes them as a plan:

```bash
python3 smart_playlists.py --export "iTunes Library.xml" --owner dunshill --out plan.json
```

It prints what each playlist asks for, and for anything that cannot cross over it says why:
Navidrome has no rule for Composer or Grouping, no way to limit a playlist to sixty minutes,
and Aurral's editor does not show nested groups. Nothing is guessed at.

`apply-smart-playlists.mjs` then applies the plan from inside the Aurral container, creating or
updating one playlist per entry:

```bash
docker exec -i -e PLAN=/tmp/plan.json -e OWNER=dunshill -e APPLY=true aurral \
  node --input-type=module - < scripts/itunes-migrate/apply-smart-playlists.mjs
```

Without `APPLY=true` it prints what it would do. A second run updates the rules on the playlist
it made the first time rather than making another.
