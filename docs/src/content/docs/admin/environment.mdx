...
| `OIDC_REQUIRED` | Make OIDC the only interactive login path. Authentication is always required, local username/password login (form, basic auth, and legacy `AUTH_USER`/`AUTH_PASSWORD`) is refused, the local-network bypass is ignored, and the sign-in page redirects straight to your IdP. Requires a working `OIDC_ENABLED` configuration; the API key and Subsonic endpoints keep working for integrations. |
...
| `AURRAL_PLAYLISTS_ENABLED` | Set to `false` to disable playlist making entirely: the `accessFlow` permission is denied for every user (admins included), which hides and blocks all playlist creation UI and routes, and the automatic discovery and weekly-flow playlist builders stop running. Default `true`. |
| `AURRAL_AUTOMATIC_PLAYLISTS_ENABLED` | Set to `false` to keep hand-made playlists but turn off everything Aurral generates on its own: flows and the discovery playlist builders stop, the Flows page and Discover playlists disappear, and the flow routes refuse changes. Default `true`. |
| `AURRAL_NAVIDROME_USER_AUTH` | Set to `reverse-proxy` to have Aurral talk to Navidrome as the signed-in Aurral user, by sending the trusted username header Navidrome's external-auth support reads. Hand-made playlists then live in Navidrome and Aurral reads and edits them there, so nothing needs syncing, and track ratings and stars are read and written in Navidrome as that user. Navidrome must list Aurral's address in its trusted sources (`ND_EXTAUTH_TRUSTEDSOURCES`, formerly `ND_REVERSEPROXYWHITELIST`), and Aurral usernames must match Navidrome usernames. Default `off`. |
| `AURRAL_NAVIDROME_USER_HEADER` | The header carrying the username when `AURRAL_NAVIDROME_USER_AUTH` is on. Must match Navidrome's `ND_EXTAUTH_USERHEADER` (formerly `ND_REVERSEPROXYUSERHEADER`). Default `Remote-User`. |
| `AURRAL_NAVIDROME_MUSIC_ROOT` | Absolute path, as Aurral sees it, of the folder Navidrome's main library indexes (for example `/data/Music/Library`). Left unset, Aurral works the mapping out from the first file it matches on both sides, which costs a title search and can be fooled by a same-named file. Setting it states the mapping outright. |
| `AURRAL_NAVIDROME_PATH_PREFIX` | Prefix Navidrome puts in front of the shared path tail, when it reports paths that are not relative to the library root. Usually empty. Only read when `AURRAL_NAVIDROME_MUSIC_ROOT` is set. |
| `AURRAL_DISCOVER_DEFAULT_SECTIONS` | Comma-separated, ordered list of home (Discover) section ids that are enabled by default for users who have not customized their layout, for example `recentlyAdded,recentReleases,recommended`. Sections not listed start disabled but users can re-enable them. Valid ids: `recentlyAdded`, `playlists`, `recommendedShows`, `recentReleases`, `news`, `recommended`, `globalTop` (Global Trending), `genreSections`. Unset keeps every section enabled. |
