...
| `AURRAL_NAVIDROME_USER_AUTH` | Set to `reverse-proxy` to have Aurral talk to Navidrome as the signed-in Aurral user, by sending the trusted username header Navidrome's external-auth support reads. Hand-made playlists then live in Navidrome and Aurral reads and edits them there, so nothing needs syncing. Navidrome must list Aurral's address in its trusted sources (`ND_EXTAUTH_TRUSTEDSOURCES`, formerly `ND_REVERSEPROXYWHITELIST`), and Aurral usernames must match Navidrome usernames. Default `off`. |
